...
function Calculator() {
  const [input, setInput] = useState("");//Returns a stateful value, and a function to update it.
  ...
  // checking brackets are balanced or not
  ...
  const calculateAns = () => {
    ...
    try {
      ...
      if (!checkBracketBalanced(finalexpression)) {
        const errorMessage = { message: "Expression error" };
        throw errorMessage;//expection is thrown
      }
